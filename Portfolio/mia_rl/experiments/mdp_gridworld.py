...
def run_mdp_gridworld():
    ...
    for i in indices_to_plot:
        ...
        plot_grid_values_and_policy(
            output_dir, 
            "plot_grid_values_and_policy_{current_it_pi}",
    ...
    plot_grid_values_and_policy(output_dir, f"plot_grid_values_and_policy_{current_it_pi}.png", env, V_pi, None, title="Policy evaluation: V^π (uniform random π)")

    plot_grid_values_and_policy(output_dir, "plot_grid_values_and_policy_value_iteration.png", env, V_star, pi_star, title="Value iteration: V* and greedy policy")
    ...
    for g in [0.5, 0.9, 0.99]:
        ...
        plot_grid_values_and_policy(output_dir, f"plot_grid_values_and_policy_{str(g).replace('.', '_')}.png", env, Vg, None, title=f"V* and greedy policy (gamma={g})")
        plot_grid_values_and_policy(output_dir, f"plot_grid_values_and_policy_value_iteration_{str(g).replace('.', '_')}_policy.png", env, Vg, pig, title=f"V* and greedy policy (gamma={g})")
    ...
    plot_grid_values_and_policy(output_dir, "plot_grid_values_and_policy_trap_gridworld.png", env_trap, V_trap, None, title="V* with trap at (0,2) reward -10")
    plot_grid_values_and_policy(output_dir, "plot_grid_values_and_policy_trap_gridworld_policy.png", env_trap, V_trap, pi_trap, title="V* with trap at (0,2) reward -10")
    ...
    plot_grid_values_and_policy(output_dir, "plot_grid_values_and_policy_stochastic.png", env, V_stoch, None, title="Stochastic: V^pi (uniform)")
    plot_grid_values_and_policy(output_dir, "plot_grid_values_and_policy_stochastic_policy.png", env, V_stoch, pi_stoch, title="V* with stochastic slip (0.8/0.1/0.1)")
