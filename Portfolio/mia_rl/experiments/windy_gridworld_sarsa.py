...
def run_windy_gridworld_sarsa():
    ...
    output_dir = Path(__file__).resolve().parents[1] / "outputs" / "windy_gridworld_sarsa"
    ...
    try:
        ...
        plot_episode_lengths(output_dir, algorithm, episode_lengths)
        plot_episode_rewards(output_dir, algorithm, episode_rewards)
        plot_policy(output_dir, algorithm, env, policy, path=path, title="Windy Gridworld greedy policy after Sarsa training")
