...
class BlackjackEnv(Environment[BlackjackState, BlackjackAction]):
    ...
    def step(self, action: BlackjackAction) -> tuple[BlackjackState, float, bool]:
        ...
        if action == "hit":
            """Handle the player taking one extra card.

            TODO:
            1. Draw one new card for the player.
            2. Compute the resulting `next_state`.
            3. If the player busts, return `(next_state, -1.0, True)`.
            4. Otherwise return `(next_state, 0.0, False)`.
            """
            if action == "hit":
                self.player.append(draw_card(self.rng))
            ...
            #raise NotImplementedError("TODO: implement the 'hit' transition in BlackjackEnv.step().")
